import { JurnalGuru, JurnalSettings } from '@/lib/jurnal-service';
...
export async function fetchJurnalSettings(): Promise<JurnalSettings> {
    ...
    return data.data; // Now safely unwrapped from { success: true, data: ... }
}
...
export async function generateJurnalMassal(
    startDate: string,
    endDate?: string,
    jamKe?: string // Changed to string for "1,2,3" support
...
export async function fetchJurnal(filter: JurnalFilter) {
    const params = new URLSearchParams();
