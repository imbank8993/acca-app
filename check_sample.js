...
async function checkData() {
    const { data: sk } = await supabase.from('siswa_kelas').select('kelas').limit(5);
    const { data: mk } = await supabase.from('master_kelas').select('nama').limit(5);

    console.log('siswa_kelas sample:', sk);
    console.log('master_kelas sample:', mk);
}
